package com.periut.starac.retrocenter.child;
 ...
 * - com.periut.starac.retrocenter.bridge.* — the hub/child coordination
 *   statics (HubBridge, ChildConfig).
 * - logging + netty — stateless-ish infrastructure, avoids double init.
 *
 * NOTE: the child's classpath must NOT contain the full starac artifact
 * (its org.lwjgl package would be self-loaded by the child's Knot and break
 * the sharing). The child instead gets the "childshim" jar — starac minus
...
public final class ChildClassLoader extends URLClassLoader {
	...
	private static final String[] SHARED_PREFIXES = {
			...
			"com.sun.",
			"org.lwjgl.",
			"com.periut.starac.retrocenter.bridge.",
			"org.apache.logging.",
			"org.slf4j.",
			"io.netty.",
	};
	...
	public ChildClassLoader(URL[] classpath, ClassLoader parent) {
		...
		this.instanceId = com.periut.starac.retrocenter.bridge.HubBridge.registerChildLoader(this);
	}
